...
\begin{document} % All begin commands must be paired with an end command somewhere
    ...
    \begin{center}
        \begin{tikzpicture}
            \node[above right,
            inner sep=0](image) at (0,0) {\includegraphics[width=0.9\textwidth]{port init.png}};
            \begin{scope}[
                x={($0.1*(image.south east)$)},
                y={($0.1*(image.north west)$)}]
                
                \draw[very thick,Purple] (0,7) rectangle (8.2,9.9)
                node[right,white,fill=Purple]{\small Port Initalization};
                \end{scope}
        \end{tikzpicture}
